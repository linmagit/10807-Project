...
tf.flags.DEFINE_integer("num_epochs", 205, "Number of training epochs (default: 200)")
...
# x_test is a list of senetences
# y is one hot representation of classes
x_text, y = data_helpers.load_custom_data(['tr_{}_2col_30k.pkl', 'tr_{}_2col_bs20_30k.pkl'])

# Build vocabulary
max_document_length = max([len(x.split(" ")) for x in x_text])
print("max document length is {}".format(max_document_length))
vocab_processor = learn.preprocessing.VocabularyProcessor(max_document_length)
# x is an array of index vectors of the vocabulary
...
x_train,y_train=x,y
x_dev_raw, y_dev=data_helpers.load_custom_data(['val_{}_2col_8k.pkl', 'val_{}_2col_bs20_8k.pkl'])
x_dev = np.array(list(vocab_processor.transform(x_dev_raw)))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        optimizer = tf.train.AdamOptimizer(5*1e-4)
        ...
        err_summary = tf.scalar_summary("error", cnn.error)

        # Train Summaries
        train_summary_op = tf.merge_summary([loss_summary, err_summary, grad_summaries_merged])
        ...
        def train_step(x_batch, y_batch):
            ...
            _, step, summaries, loss, accuracy, error = sess.run(
                [train_op, global_step, train_summary_op, cnn.loss, cnn.accuracy, cnn.error],
                feed_dict)
...
#            print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy)) 
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            if epoch % FLAGS.evaluate_every == 0:
                ...
                time_str = datetime.datetime.now().isoformat()
                print("{}: training loss: {}, error:{} Testing loss: {}, error:{}".format(epoch, tr_loss, tr_error, te_loss, te_error))
